		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" @click="editPackage()">
						Save Changes
					</button>
	...
	methods: {
		...
		updateTo(index) {
			if (Number(this.durations[index].to) > 35) {
				...
				if (
					Number(this.durations[index].duration_to) <= Number(this.durations[index].duration_from)
				) {
					this.durations[index].duration_to = Number(this.durations[index].duration_from) + 1;
				}
			}
		},
		removeDuration() {
